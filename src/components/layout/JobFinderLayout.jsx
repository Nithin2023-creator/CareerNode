    ...
    <div className="space-y-10">
      ...
      <div className="flex flex-row justify-between items-start gap-4">
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="min-w-0">
          ...
          <h1 className="font-display text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold tracking-tight uppercase leading-[0.9]">
            Job Finder.
          </h1>
        ...
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-2 shrink-0">
          {showIntroTrigger && (
            <>
              {/* Icon-only on mobile, pill label on sm+ */}
              <button
                type="button"
                onClick={() => setIsIntroModalOpen(true)}
                aria-label="How Job Finder works"
                className="sm:hidden h-10 w-10 rounded-full border border-black/10 bg-white hover:bg-black/5 flex items-center justify-center transition-colors shrink-0"
              >
          ...
          {/* Wallet — icon only on mobile, pill on sm+ */}
          <NavLink
            to="/dashboard/job-finder/wallet"
            className="flex sm:hidden h-10 w-10 rounded-full bg-[var(--color-accent-yellow)]/20 text-black hover:bg-[var(--color-accent-yellow)]/40 transition-colors items-center justify-center shrink-0"
            aria-label={`Wallet balance: ${wallet.balance} credits`}
          >
            <Wallet className="h-4 w-4" />
          </NavLink>
          <NavLink
            to="/dashboard/job-finder/wallet"
            className="hidden sm:flex pill-badge bg-[var(--color-accent-yellow)]/20 text-black hover:bg-[var(--color-accent-yellow)]/40 transition-colors h-10 md:h-12 items-center shrink-0"
          >
          ...
          <button
            ...
            className="h-10 w-10 md:h-12 md:w-12 rounded-full bg-black text-white hover:bg-[var(--color-accent-blue)] flex items-center justify-center transition-colors relative shrink-0"
          >
